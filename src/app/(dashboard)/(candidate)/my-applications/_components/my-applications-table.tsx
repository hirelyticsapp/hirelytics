...
export default function MyApplicationsTable() {
  ...
  const { data, totalCount, pageCount, isLoading, error, refetch } = useDataTable({
    queryKey: ['my-applications', pagination, filters, sorting],
  ...
  const columns: ColumnDef<JobApplicationTableData>[] = [
    {
      ...
      cell: ({ row }) => (
        <div className="max-w-[250px] truncate" title={row.original.jobDetails.title}>
          {row.original.jobDetails.title}
        </div>
      ),
    },
    {
      accessorKey: 'jobDetails.skills',
      header: 'Skills',
      cell: ({ row }) => (
        <div className="flex flex-wrap gap-1 max-w-[200px]">
          {row.original.jobDetails.skills.slice(0, 3).map((skill, index) => (
            <Badge key={index} variant="outline" className="text-xs">
              {skill}
            </Badge>
          ))}
          {row.original.jobDetails.skills.length > 3 && (
